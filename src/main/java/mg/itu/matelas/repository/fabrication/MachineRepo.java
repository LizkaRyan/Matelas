...
public interface MachineRepo extends JpaRepository<Machine,Long> {
    @Query("select new mg.itu.matelas.entity.fabrication.Machine(ma.idMachine,ma.machine,avg(mv.ecart)) from MvtStock mv join mv.machine ma group by ma.idMachine,ma.machine order by mv.ecart asc")
