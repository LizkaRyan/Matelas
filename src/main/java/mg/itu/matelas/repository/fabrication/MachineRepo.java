...
public interface MachineRepo extends JpaRepository<Machine,Long> {
    ...
    @Query("select new mg.itu.matelas.entity.fabrication.Machine(ma.idMachine,ma.machine,AVG(mv.ecart),sum(mv.prixRevient),sum(mv.prixRevient-(mv.ecart*mv.matelas.longueur*mv.matelas.largeur*mv.matelas.epaisseur)),sum(mv.matelas.longueur*mv.matelas.largeur*mv.matelas.epaisseur)) from MvtStock mv join mv.machine ma where extract(year from mv.dateMvtStock)=:annee group by ma.idMachine,ma.machine order by AVG(mv.ecart) asc")
